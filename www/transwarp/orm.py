...
class Field(object):
    ...
    def __str__(self):
        s = ['<{0}:{1},{2},default({3}),'.format(self.__class__.__name__,
...
def _gen_sql(table_name, mappings):
    ...
    for f in sorted(mappings.values(), key=lambda x: x._order):
        if not hasattr(f, 'ddl'):
            raise Exception('no ddl in field "%s".' % n)
        ddl = f.ddl
        nullable = f.nullable
        if f.primary_key:
            pk = f.name
...
class ModelMetaclass(type):
    ...
    name:类名
    bases:类继承的父类集合
    classdict:key为类属性和方法的名字，value为属性和方法对象
    '''
    def __new__(cls, name, bases, classdict):
        ...
        # 类字段属性均已保存到mappings中，因此将其从类中删除，避免混淆
        for k in mappings.keys():
            classdict.pop(k)
        # 未制定表名则以类名作为表名
        if not '__table__' in classdict:
            classdict['__table__'] = name.lower()
        classdict['__mappings__'] = mappings
        classdict['__primary_key__'] = primary_key
        classdict['__sql__'] = lambda self: _gen_sql(classdict['__table__'], mappings)
        for trigger in _triggers:
            if not trigger in classdict:
                classdict[trigger] = None
        return type.__new__(cls, name, bases, classdict)
...
class Model(dict, metaclass = ModelMetaclass):
    ...
    def find_first(cls, where, *args):
        '''
        Find by where clause and return one result. If multiple results
        found,only the first one returned. If no result foound, return None.
        '''
...
if __name__=='__main__':
    ...
    db.create_engine('root', '7780105', 'test')
